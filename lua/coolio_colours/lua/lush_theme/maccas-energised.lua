...
local theme = lush(function(injected_functions)
	...
	local yellow_mac_punchy = hsl("#d32c3b")
	-- local green_mac         = hsl("#add304")
	local green_mac = hsl("#ffef00")
	-- local green_mac_light   = hsl("#eaff61")
	local green_mac_light = hsl("#8aa961")
	return {
		...
		CursorLine({ bg = orange_mac.da(30).ro(-10) }), -- Screen-line at the cursor, when 'cursorline' is set. Low-priority if foreground (ctermfg OR guifg) is not set.
		...
		--
		-- See :h group-name
		Comment({ fg = orange_mac, gui = "italic" }), -- Any comment
		...
		sym("@variable")({ fg = Normal.fg.li(20), gui = "bold" }), -- Identifier
		-- sym"@type"              { }, -- Type

...
local theme = lush(function(injected_functions)
	...
	return {
		...
		CursorLine({ bg = yellow_mac2.li(40) }), -- Screen-line at the cursor, when 'cursorline' is set. Low-priority if foreground (ctermfg OR guifg) is not set.
		CursorColumn({ CursorLine }), -- Screen-column at the cursor, when 'cursorcolumn' is set.
		Directory({ fg = yellow_mac2.da(70).ro(60), gui = "bold" }), -- Directory names (and other special names in listings)
		...
		FoldColumn({ fg = yellow_mac, bg = red_mac_black }), -- 'foldcolumn'
		...
		LineNr({ fg = green_mac, bg = yellow_mac2, gui = "bold" }), -- Line number for ":number" and ":#" commands, and when 'number' or 'relativenumber' option is set.
		...
		Title({ fg = green_mac_light, gui = "bold" }), -- Titles for output from ":set all", ":autocmd" etc.
		Visual({ fg = red_mac, bg = Normal.fg }), -- Visual mode selection
		...
		--
		-- See :h group-name
		Comment({ fg = yellow_mac.li(40), gui = "italic" }), -- Any comment

		Constant({ fg = red_mac_black.ro(90).da(50) }), -- (*) Any constant
		...
		Identifier({ fg = red_mac_black.ro(10) }), -- (*) Any variable name
		Function({ fg = yellow_mac.ro(20).sa(90), gui = "bold" }), --   Function name (also: methods for classes)

		Statement({}), -- (*) Any statement
		Conditional({ fg = yellow_mac, gui = "bold" }), --   if, then, else, endif, switch, etc.
		Repeat({ Conditional }), --   for, do, while, etc.
		Label({ Conditional }), --   case, default, etc.
		Operator({ fg = yellow_mac2.da(70).ro(60) }), --   "sizeof", "+", "*", etc.
		...
		Type({ fg = yellow_mac }), -- (*) int, long, char, etc.
		...
		Todo({ bg = yellow_mac_punchy, fg = red_mac }), -- Anything that needs extra attention; mostly the keywords TODO FIXME and XXX
		...
		sym("@variable")({ fg = Identifier.fg.ro(-100) }), -- Identifier
		-- sym"@type"              { }, -- Type
